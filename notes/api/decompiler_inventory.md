...
- `Funcdata::getSymbol()` returns `FunctionSymbol*` linking back to the scope/database.
...
- `Symbol::category` values: `-1` = no category, `0` = function_parameter, `1` = equate, `2` = union_facet. Use `getCategorySize(0)` to count function parameters.
...
| `Funcdata::numCalls()` / `getCallSpecs(i)` | Call sites within the function | `int4` count, `FuncCallSpecs*` | `int4 numCalls() const`, `FuncCallSpecs* getCallSpecs(int4 i) const` | Pointers owned by `Funcdata` | No throws | Read-only | `optional` |
...
| `Comment::getType()` / `getText()` / `getAddr()` | Individual comment/warning data | `uint4` type flags, `const string&` text, `const Address&` | Inline accessors | Stable while database lives | No throws | Read-only | `required` |
| `LowlevelError` / `RecovError` / `ParseError` | Exception hierarchy thrown by decompile path | `string explain` (error message) | Caught by value or reference at call boundary | Stack-based; catch at boundary | N/A (they ARE the error) | N/A | `required` |
...
- There is no built-in timeout/budget mechanism in the public API. Timeouts must be implemented externally (e.g., signal/thread cancellation). The `max_instructions` field on `Architecture` limits instruction count per function.
