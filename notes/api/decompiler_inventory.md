...
- Inventory content is now strictly upstream Ghidra-only (no adapter-specific notes).
- Corrected visibility/ABI detail: `SleighArchitecture::collectSpecFiles(ostream&)` is `protected static`, not a public API entrypoint.
- Corrected language-enumeration API: upstream exports `SleighArchitecture::getDescriptions(void)`.
- Added missing function-materialization path needed by a standalone caller that only has an entry address: `Scope::addFunction(const Address&, const string&)` and `FunctionSymbol::getFunction(void)`.
- Removed/de-scoped non-minimal internal items from the callable surface (`Architecture::clearAnalysis`, `ActionDatabase::universalAction`, and internal subsystem details not directly invoked by driver code).
...
| `shutdownDecompilerLibrary(void)` | `libdecomp.hh` / `libdecomp.cc` | function | In: none. Out: library shutdown hook (currently no-op in pinned source). | Process-global hook. | Called at plugin/library teardown. | No throws in current implementation. | Same global lifecycle constraints as startup. | `required` | `Ghidra_12.0.3_build` |
| `SleighArchitecture::scanForSleighDirectories(const string&)` | `sleigh_arch.hh` / `sleigh_arch.cc` | static function | In: root path. Out: appends discovered language directories to static `specpaths`. | Global mutable path registry. | Before `getDescriptions`/`resolveArchitecture`. | Path discovery failures surface later during spec load/resolve. | Not safe for concurrent mutation. | `required` | `Ghidra_12.0.3_build` |
...
| `SleighArchitecture::shutdown(void)` + `SleighArchitecture::specpaths` | `sleigh_arch.hh` / `sleigh_arch.cc` | static function + static data | Used to reset language-discovery globals before re-scanning paths. | Process-global mutable state. | If reconfiguring language roots at runtime, clear/reset before calling `scanForSleighDirectories` again. | `shutdown()` is non-throwing in pinned source. | Global mutable state requires external synchronization. | `optional` | `Ghidra_12.0.3_build` |
