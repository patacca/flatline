...
- Audit note: there are no file-level diffs under `Ghidra/Features/Decompiler/src/decompile/cpp`
  between `Ghidra_12.0.3_build` and `Ghidra_12.0.4_build`, so the callable surface
  catalog below remains unchanged aside from the upstream pin.
