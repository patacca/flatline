...
- Next: P6 host-feasibility work is now macOS-first, Windows-second per ADR-008; `docs/host_feasibility.md` records the current audit, the pinned `macos-15` native contract lane, and the equivalent contract-coverage bar; P7 enriched output remains deferred behind ADR-012.
...
- Workflow specs: `tests/unit/test_ci_workflow_spec.py` pins CI runner/env matrix, regression gate, and macOS native contract lane; `tests/unit/test_native_tox_env_spec.py` pins the native-forced tox env used by the macOS host-feasibility lane; `tests/unit/test_release_ci_workflow_spec.py` pins release publish workflow.
