...
- Next: P6 host-feasibility work is now macOS-first, Windows-second per ADR-008; `docs/host_feasibility.md` records the current audit, the dedicated macOS native contract lane, and the equivalent contract-coverage bar; P7 enriched output remains deferred behind ADR-012.
...
- Packaging/test shape: `py313`/`py314` tox envs build/install `flatline[test]` wheels inside `.tox`; `py314-native` forces `native_bridge=enabled` during tox packaging for the macOS host-feasibility lane; `lint` is package-skip + `ruff`; repo-only compliance/footprint/release/artifact tools live in `tools/flatline_dev/` with `tools/*.py` wrappers and are excluded from wheels/sdists by `tools/prune_dist.py`.
- P6 build hardening: `src/flatline/meson.build` now keeps compiler-family warning/visibility flags in Meson, uses Meson include directories for staged nanobind headers, and keeps macOS `zlib` discovery inside the build layer so shared native-build paths no longer depend on raw GCC-style include flags before Windows feasibility work starts.
- CI: `.github/workflows/ci.yml` uses latest stable Python `3.14` for non-Ubuntu jobs (`lint`, `build`, current macOS native contract lane, release workflow build); Ubuntu keeps the full supported matrix (`py313` + `py314`) for `test`, and `regression` runs `py314`; the macOS lane uses `tox -e py314-native -- -m "not regression"` with Homebrew `zlib` but no manual compiler/linker env flags.
...
- Specs: `tests/specs/test_catalog.md` (48 definitions, 5 categories, traceability matrix), `tests/specs/fixtures.md` (10 fixtures, oracle strategy, determinism rules).
- Workflow/build specs: `tests/unit/test_ci_workflow_spec.py`, `tests/unit/test_native_tox_env_spec.py`, and `tests/unit/test_release_ci_workflow_spec.py` smoke-check critical CI/native-lane/publish invariants without treating incidental workflow structure as a stable interface.
