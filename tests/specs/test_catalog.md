...
| U-025 | Validate release publish workflow smoke invariants | Committed `.github/workflows/release.yml` | Parse the committed release workflow definition | The workflow runs only on GitHub `release.published` or `workflow_dispatch`, still builds and audits wheel+sdist artifacts, and routes manual dispatches to TestPyPI while release-triggered publishes go to PyPI | Workflow-smoke oracle | Critical publish routing and artifact validation stay explicit without pinning incidental step structure |
...
| §7 Cross-platform feasibility policy | A dedicated macOS feasibility lane forces native builds and runs the installed-wheel non-regression contract matrix before support notes can change, while shared native-build paths stay host-neutral before Windows work begins | U-026 |
