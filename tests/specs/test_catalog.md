...
| U-021 | Validate release readiness audit happy path | Synthetic repo with version files and release docs | Audit the release-readiness helper against a compliant synthetic repo | The active `0.1.1.dev1` release candidate collapses to `0.1.1` with all required artifacts present | Release-readiness audit oracle | The current production-publish recommendation and required artifact enumeration remain explicit |
...
| §3.5 pre-1.0 release-line policy + §7 release workflow | The current `.devN` release candidate collapses to the documented public version, and the release workflow stays source-controlled and auditable | U-021 |
