...
| U-019 | Validate CI runner and Python-version policy | Committed `.github/workflows/ci.yml` | Parse the committed CI workflow definition | Ubuntu test lanes cover all supported Python versions (3.13/3.14), while non-Ubuntu jobs stay on the latest stable Python (`3.14`); perf-sensitive test/regression lanes pin `ubuntu-24.04`; the dedicated regression lane runs `tox -e py314 -- -m regression` against the installed wheel artifact | Workflow-structure oracle | Runner pinning and per-OS Python policy remain source-controlled |
...
| U-024 | Validate repo-only dev-tool layout | Committed `src/flatline/` + `tools/flatline_dev/` + `tools/*.py` wrappers | Inspect the maintained repo layout for shipped-package boundaries | Repo-only compliance/footprint/release/artifact helpers live under `tools/flatline_dev` with wrapper entrypoints in `tools/`; no legacy dev-only helpers leak back into `src/flatline` | Layout oracle | Dev/release helpers stay physically separated from the shipped package tree |
| U-025 | Validate release publish workflow structure | Committed `.github/workflows/release.yml` | Parse the committed release workflow definition | The workflow runs only on GitHub `release.published` or `workflow_dispatch`, builds/audits wheel+sdist artifacts on the latest stable Python (`3.14`), and routes manual dispatches to TestPyPI while release-triggered publishes go to PyPI via trusted publishing | Workflow-structure oracle | Release automation remains source-controlled, artifact validation stays explicit, and manual publishes never target production PyPI |
| U-026 | Validate native-forced tox env and pinned macOS contract lane | Committed `pyproject.toml` + `.github/workflows/ci.yml` | Parse the native tox config and the macOS CI job | `py314-native` forces `native_bridge=enabled`, and the pinned `macos-15` lane runs `tox -e py314-native -- -m "not regression"` on Python `3.14` without manual `CPPFLAGS` / `LDFLAGS` / `PKG_CONFIG_PATH` exports | TOML/YAML structure oracle | Host-feasibility CI never silently degrades to the Python fallback bridge or low-level manual flag plumbing |
...
| §7 Packaging and compliance | Release artifacts include notices and attribution; ADR-007 audit passes; built wheel/sdist metadata stay auditable; repo-only dev tooling stays outside shipped payloads; default-install footprint stays measured/documented with an explicit size-policy note | U-017, U-018, U-022, U-024 |
...
| §7 Release automation | GitHub Actions release automation builds and audits distributions, publishes GitHub releases to PyPI, and routes manual dispatches to TestPyPI | U-025 |
| §7 Cross-platform feasibility policy | The pinned macOS feasibility lane forces native builds and runs the installed-wheel non-regression contract matrix before support notes can change | U-026 |
