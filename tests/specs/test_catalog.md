...
| U-025 | Validate release publish workflow structure | Committed `.github/workflows/release.yml` | Inspect the committed release workflow definition | The workflow runs only on GitHub `release.published` or `workflow_dispatch`, builds/audits wheel+sdist artifacts, and routes manual dispatches to TestPyPI while release-triggered publishes go to PyPI via trusted publishing | Workflow-text oracle | Release automation remains source-controlled, artifact validation stays explicit, and manual publishes never target production PyPI |
