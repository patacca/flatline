...
| U-028 | Validate enriched-output bridge enforcement | None | Adapt native enriched payloads through the bridge; simulate a successful native response that omits the opt-in payload | The bridge serializes the opt-in flag, coerces the nested `Enriched` / `Pcode` payload to public value types, and successful opt-in requests fail hard if `enriched` or `enriched.pcode` is missing | Boundary-shape oracle | Opt-in enriched output never silently disappears on successful results |
| U-029 | Validate bridge coerces instruction payloads | None | Adapt native instruction dicts through bridge adapter | Bridge coerces instruction list to public InstructionInfo value types | Boundary-shape oracle | Instruction coercion stays deterministic |
| U-030 | Validate bridge rejects malformed instruction payloads | None | Feed malformed instruction dicts through bridge adapter | Malformed instruction dicts raise InternalError | Error-envelope oracle | Malformed instruction rejection is deterministic |
| U-029 | Validate pcode lookup and graph projection | Synthetic pcode payload | Look up pcode ops and varnodes by ID, project them into a graph, and try malformed IDs | `Pcode.get_pcode_op()` / `get_varnode()` provide deterministic ID lookup, `Pcode.to_graph()` returns a deterministic `networkx.MultiDiGraph`, duplicate operand slots remain distinct multiedges, and malformed IDs fail with `invalid_argument` | Graph-projection oracle | Graph traversal inputs stay deterministic and repeated operand slots do not collapse |
| U-040 | Validate bridge populates enriched instructions from double | None | Use bridge double with instructions in enriched payload; run bridge adapter | Enriched.instructions is populated from the native payload | Boundary-shape oracle | Enriched instructions population stays deterministic |
| U-031 | Validate Windows native DLL bootstrap for unrepaired source-built wheels | Synthetic `VCPKG_INSTALLATION_ROOT` tree with/without `zlib1.dll` | Run the Windows DLL bootstrap helper under simulated `win32` imports | When `zlib1.dll` exists under `installed/x64-windows/bin`, the package registers that directory via `os.add_dll_directory()`; when the DLL is absent, the bootstrap stays silent | Runtime-bootstrap oracle | Windows source-built native imports remain deterministic without affecting repaired wheels or non-Windows platforms |
...
| C-007 | Enriched-output schema stability | None | Inspect `DecompileRequest`, `DecompileResult`, and enriched companion dataclasses | The opt-in request flags plus `Enriched`, `Pcode`, `PcodeOpInfo`, `VarnodeInfo`, and `VarnodeFlags` keep stable field names | JSON-schema-like oracle | Additive fields allowed only; enriched companion field names are contract-stable |
| C-008 | Enriched-output graph surface stability | None | Inspect `Pcode` helper methods and signatures | `get_pcode_op(op_id)`, `get_varnode(varnode_id)`, and `to_graph()` remain available with stable parameter names | API-surface oracle | Graph helper names and parameter names remain stable |
| C-009 | InstructionInfo schema stability | None | Inspect InstructionInfo dataclass fields and types | address (int), length (int), mnemonic (str), operands (str) are stable | JSON-schema-like oracle | Additive fields allowed only |
| C-010 | Enriched.instructions field stability | None | Inspect Enriched.instructions field type | Field exists with type list[InstructionInfo] | None | API-surface oracle | Field name and type are contract-stable |
...
| I-008 | Opt-in enriched output supports graph analysis | `fx_add_elf64` | Decompile the known add fixture with `enriched=True`, then traverse `result.enriched.pcode.to_graph()` from the exported `INT_ADD` result | Default requests keep `enriched=None`; opt-in requests populate `enriched.pcode`; graph reachability from the `INT_ADD` result reaches a `RETURN` op through exported use-def edges | Use-def traversal oracle | Pcode-op IDs, varnode edges, and graph projection stay deterministic for the pinned fixture/upstream pair |
...
| I-010 | Tail-padding toggle preserves strict failure mode for exact slices with external calls | `fx_external_call_arm64` trimmed to its committed function size | Decompile with default tail padding, with a custom non-empty padding pattern, and with disabled padding | Default and custom-padding requests succeed with non-empty C output despite call targets outside the supplied span; disabled padding returns structured `invalid_address` | Dual-path status oracle | Caller convenience stays default; strict opt-out and custom padding remain deterministic |
...
| R-005 | Preserve branch delay-slot alias enrichment | `fx_delay_slot_branch_mips32` | Decompile fixture with `enriched=True` and inspect pcode ops | Normalized C output and `varnode_count` stay stable; enriched pcode includes canonical `MULTIEQUAL` after `BUILD` alias normalization | Canonical token stream + enriched-opcode oracle | Placeholder alias handling remains stable for the pinned Ghidra revision |
| R-006 | Preserve call delay-slot alias enrichment | `fx_delay_slot_call_mips32` | Decompile fixture with `enriched=True` and inspect pcode ops | Normalized C output and `varnode_count` stay stable; enriched pcode includes canonical `INDIRECT` after `DELAY_SLOT` alias normalization | Canonical token stream + enriched-opcode oracle | Placeholder alias handling remains stable for the pinned Ghidra revision |
...
| §3.3 DecompileRequest `enriched` | Opt-in enriched output is explicit and defaults to `False` | U-028, C-007, I-001, I-008 |
...
| §3.3 DecompileResult `enriched` | Companion payload is absent by default and present on successful opt-in requests | U-028, C-001, C-007, I-001, I-008 |
...
| §3.3 Enriched, Pcode, PcodeOpInfo, VarnodeInfo, VarnodeFlags | Frozen companion schema and graph edges remain stable | U-028, U-029, C-007, C-008, I-008 |
| §3.3 InstructionInfo + Enriched.instructions | Native instruction payloads and enriched instruction lists remain stable | U-029, U-030, U-040, C-009, C-010 |
| §3.3 Pcode graph helpers | ID lookup and graph projection remain stable and deterministic | U-029, C-008, I-008 |
...
| §3.4 Invalid address → hard error | Not warning-only outside the request's effective mapped span | N-001, I-010 |
...
| §3.4 Pcode lookup input validation | Malformed or unknown pcode/varnode IDs fail hard | U-029 |
