...
| ADR-004 Runtime Asset Policy | Given ADR-010's `ghidra-sleigh` distribution mechanism, which asset profile is required or recommended for flatline by default? Must address: priority-ISA minimum coverage, whether the default install expects the all-processors build or a lighter profile, package size budget, compatible `ghidra-sleigh` version pinning, and any optional/custom runtime-data policy. | End of P2 |
...
| ADR-010 Runtime Data Packaging | How are compiled `.sla` and runtime data files packaged and distributed? **Decided: separate `ghidra-sleigh` pip package** (import `ghidra_sleigh`). It builds `sleighc` from Ghidra C++ sources at package build time, compiles `.sla` files ahead of use, ships them as package data, and exposes `ghidra_sleigh.get_runtime_data_dir()` for consumers. This resolves the distribution mechanism only; default asset-profile, size-budget, and optional-dependency policy questions remain tracked in ADR-004. See `patacca/ghidra-sleigh`. | Start of P2 (decided) |
